...
- `run_eval.py` — Structured metrics + pairwise preference judging + report generation. (~4121 tok)
...
- `run_eval_judge_sanity.py` — Experiment 003 Phase 0.1 — judge self-preference bias sanity check. (~3345 tok)
...
- `judge_backend.py` — Judge backend dispatch. (~1732 tok)
- `judge_pairwise.py` — Pairwise preference judging with position-bias detection and multi-judge support. (~2601 tok)
...
- `test_judge_backend.py` — Tests for judge_backend — model-id classification and dispatch contract. (~2038 tok)
- `test_judge_pairwise.py` — Tests for pairwise judging — parsing, position bias, multi-judge. (~2371 tok)
