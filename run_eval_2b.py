"""Experiment 2B eval — cross-generation 4.6 × 4.7.

Filters corpus to 3 complex tasks, points run_eval machinery at the 2B
matrix dir. C(3, 2) = 3 pairs × 3 tasks × 2 judges × 2 orderings = 36 LLM
calls.

Run:     uv run --active python run_eval_2b.py
Resume:  Same command.
"""
...
run_eval.MATRIX_DIR = Path("logs/matrix_2b_crossgen")
run_eval.EVAL_DIR = Path("logs/eval_2b")
...
async def main() -> None:
    ...
    logger.info(
        ...
        len(config_ids), len(tasks), len(config_ids) * len(tasks),
    )
    if metrics_only:
